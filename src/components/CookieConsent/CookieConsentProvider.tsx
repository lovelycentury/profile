...
export function CookieConsentProvider({ children, config }: CookieConsentProviderProps) {
  ...
  const effectiveGoogleConsentMode = React.useMemo((): GoogleConsentModeConfig | undefined => {
  ...
  React.useEffect(() => {
    ...
    const isExpired = stored?.expiresAt
      ...
      : stored?.lastUpdated
        ? new Date(stored.lastUpdated).getTime() + (config.expirationDays ?? 365) * 86400000 <
          Date.now()
        : false;

    if (stored && stored.consentVersion === config.consentVersion && !isExpired) {
  ...
  const rejectAll = React.useCallback(async () => {
    await saveAndTrack(getDefaultCategories(config.categories), "reject_all");
  }, [config.categories, saveAndTrack]);
